...
class BlindPathNavigator:
    ...
    def _detect_path_and_crosswalk(self, image: np.ndarray) -> Tuple[Optional[np.ndarray], Optional[np.ndarray]]:
        """检测盲道和斑马线，统一NavSegInferInput/Output接口"""
        ...
        # 构造NavSegInferInput
        try:
            from yoloe_backend import NavSegInferInput, NavSegInferOutput, decode_mask
            infer_input = {
            ...
            infer_output = self.yolo_model.infer(infer_input)
            blind_path_mask = None
            crosswalk_mask = None
            for obj in infer_output["objects"]:
                mask = decode_mask(obj["mask_encoded"], obj["mask_shape"], obj["mask_encoding"])
                if obj["class_id"] == 1:
                    if blind_path_mask is None:
                        blind_path_mask = mask
                    else:
                        blind_path_mask = cv2.bitwise_or(blind_path_mask, mask)
                elif obj["class_id"] == 0:
                    if crosswalk_mask is None:
                        crosswalk_mask = mask
                    else:
                        crosswalk_mask = cv2.bitwise_or(crosswalk_mask, mask)
